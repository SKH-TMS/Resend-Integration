  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (data.success) {
          // Extract roles from the response, ensuring it's an array.
          const roles = data.user.userRoles || [];
          if (roles.length > 0) {
            console.log("User is related to team");
          } else {
            console.log("User does not have a team role");
            setIsAuthenticated(false);
            setErrorMessage("Unautherized Acces You are not in Any team");
            toast.error("Unautherized Acces You are not in Any team");
