...
function UpdateUsersInner() {
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          console.log("unautherized accesss");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.back(); // Navigate back or refresh
      } else {
  ...
  return (
    <div>
      <div className="p-4">
        <h1 className="text-2xl font-bold mb-4">Update Participants</h1>

        {users.length === 0 && !loading ? (
          <p className="text-gray-500">No Participants selected or found.</p>
        ) : (
        ...
        {users.length > 0 && (
          ...
          >
            {loading ? "Updating..." : "Update Selected Participants"}
          </button>
        )}
      </div>
    </div>
