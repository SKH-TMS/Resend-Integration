...
function UpdateUsersInner() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.push("AllProjectManagers"); // Navigate back or refresh
      } else {
  ...
  return (
    <div>
      <div className="p-4">
        <h1 className="text-2xl font-bold mb-4">Update ProjectManager</h1>

        {users.length === 0 && !loading ? (
          <p className="text-gray-500">No ProjectManager selected or found.</p>
        ) : (
        ...
        {users.length > 0 && (
          ...
          >
            {loading ? "Updating..." : "Update Selected ProjectManagers"}
          </button>
        )}
      </div>
    </div>
