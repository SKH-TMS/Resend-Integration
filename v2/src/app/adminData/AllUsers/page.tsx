...
export default function AllUsers() {
  ...
  const handleDelete = async () => {
    ...
    const confirmDelete = confirm(
      `Are you sure you want to delete ${selectedUseremails.length} selected user(s)?`
    );
    ...
    try {
      ...
      if (data.success || response.status === 207) {
        ...
        setUsers((prevUsers) =>
          prevUsers.filter((user) => !selectedUseremails.includes(user.UserId))
        );
  ...
  const handleUpdate = () => {
    ...
    router.push(
      `/adminData/UpdateUsers?emails=${selectedUseremails.join(",")}`
    );
  };
  ...
  const handleAssignProjectManager = async () => {
    ...
    const confirmAssign = confirm(
      `Assign Project Manager role to ${selectedUseremails.length} selected user(s)?`
    );
                  ...
                  <tr key={user.UserId} className="hover:bg-gray-50">
                    ...
                    <td className="border px-4 py-2 text-center align-middle">
                      <input
                        ...
                        onChange={() => handleCheckboxChange(user.email)} // Use UserId
                        checked={selectedUseremails.includes(user.email)} // Use UserId
        ...
        {users.length > 0 && (
          <div className="mt-6 flex flex-wrap gap-3 border-t pt-6">
            <button
              ...
              disabled={selectedUseremails.length === 0 || loading}
