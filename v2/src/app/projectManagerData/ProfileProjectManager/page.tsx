    ...
    const fetchUserData = async () => {
      ...
      try {
        const response = await fetch("/api/auth/token", {
          ...
          body: JSON.stringify({ userType: "ProjectManager" }),
        });
        const data = await response.json();
        if (data.success && data.ProjectManager) {
          setIsAuthenticated(true);
          const user = data.ProjectManager;
          if (!user.UserId) {
        ...
        toast.error(`Error: it is ${error.message}`);
  ...
  return (
    <div className="min-h-screen  py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Project Manager Navigation Buttons Section */}
        <div className="bg-[#ecfbf4] shadow-lg rounded-lg p-6 border border-gray-200 mt-8">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <button
              onClick={() =>
                router.push(
                  "/projectManagerData/taskManagementData/ManageTasks"
                )
              }
            ...
            >
              <RectangleStackIcon className="w-5 h-5 mr-2" /> Manage Tasks
            </button>
            <button
              onClick={() =>
                router.push(
            ...
            >
              <UserGroupIcon className="w-5 h-5 mr-2" /> Manage Teams
            </button>
          </div>
        </div>
