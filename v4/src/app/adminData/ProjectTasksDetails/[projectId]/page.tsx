...
import {
  ArrowLeftIcon,
  ClipboardDocumentIcon, // Icon for Project
  ListBulletIcon, // Icon for Tasks
  ...
  ClockIcon,
  XCircleIcon,
  LinkIcon, // For GitHub link
  ChatBubbleBottomCenterTextIcon, // For Context
  PaperClipIcon, // Generic Icon
...
interface ProjectDetailsData {
  ...
  description: string;
  status: string;
  createdBy: string; // Assuming UserId string
...
interface TaskDetailsData {
  ...
  title: string;
  description: string;
  assignedTo: AssigneeProfile[]; // Populated assignees
  ...
  gitHubUrl?: string;
  context?: string;
  submittedby?: string | AssigneeProfile | null; // Could be string, populated user, or null
}
  ...
  return (
    <header className="bg-gradient-to-r from-blue-100 to-white mb-10 p-6 rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="text-xs text-gray-500 border-t border-gray-100 pt-3 mt-3 flex items-center justify-between flex-wrap gap-2">
        ...
        {/* Optionally display createdBy if needed */}
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setTasks(data.tasks || []); // Default to empty array
        } else {
          throw new Error(
